-- ===========================================
-- Multi-Agent Architecture Migration (Minimal)
-- Only creates core tables, skips non-existent tables
-- ===========================================
...
-- 2. Add agent limit columns to accounts table
DO $$
BEGIN
...
-- 4. Add agent_id to existing tables (only if they exist)
DO $$
BEGIN
    -- Check and add to conversations table
    ...
    -- Check and add to functions table
    ...
    -- Check and add to knowledge_base table
    ...
    -- Check and add to ghl_integrations table
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='ghl_integrations') THEN
...
-- 6. Link existing data to default agents (only for tables that exist)
DO $$
BEGIN
    -- Update conversations if table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='conversations') THEN
    ...
    -- Update functions if table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='functions') THEN
        UPDATE functions f
        SET agent_id = (
            SELECT a.id
    ...
    -- Update knowledge_base if table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='knowledge_base') THEN
    ...
    -- Update ghl_integrations if table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='ghl_integrations') THEN
...
SELECT
    ...
    (SELECT COUNT(*) FROM information_schema.columns WHERE table_name='accounts' AND column_name='max_agents') as accounts_column_added;
